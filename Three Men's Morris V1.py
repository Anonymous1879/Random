...
def draw_menu():
    ...
    text1 = font.render("2 Player", True, (0, 0, 0))
    text2 = font.render("Computer", True, (0, 0, 0))
    pygame.draw.rect(screen, (200, 200, 200), (50, 100, 200, 50))
    pygame.draw.rect(screen, (200, 200, 200), (50, 200, 200, 50))
    screen.blit(text1, (75, 110))
    screen.blit(text2, (75, 210))
    pygame.display.update()

def handle_menu_input(pos):
    global game_mode
    if 50 <= pos[0] <= 250 and 100 <= pos[1] <= 150:
        game_mode = "2 player"
    elif 50 <= pos[0] <= 250 and 200 <= pos[1] <= 250:
...
def no_valid_moves(player):
    for i, spot in enumerate(board):
        if spot == player:
            adjacent_indices = []
            if i % 3 > 0 and board[i - 1] == ' ':
                adjacent_indices.append(i - 1)
            if i % 3 < 2 and board[i + 1] == ' ':
                adjacent_indices.append(i + 1)
            if i // 3 > 0 and board[i - 3] == ' ':
                adjacent_indices.append(i - 3)
            if i // 3 < 2 and board[i + 3] == ' ':
                adjacent_indices.append(i + 3)
...
# Wait for user input to choose game mode
...
while True:
    if game_mode == "2 player":
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and current_turn == player_turn:
                ...
                if index is not None:
                    if phase == 'placement':
                        if place_piece(index):
                            if check_win() or no_valid_moves(player_turn):
                                draw_pieces()
                                pygame.display.update()
                                print(f"Player {current_turn} wins!")
                                pygame.time.wait(2000)
        ...
        if current_turn == ai_turn:
            ...
            ai_move()
            if check_win() or no_valid_moves(ai_turn):
                draw_pieces()
                pygame.display.update()
                print(f"Player {current_turn} wins!")
                pygame.time.wait(2000)
